...
function createItineraryCard(itinerary) {
    ...
    // 生成标签HTML
    const tagsHtml = itinerary.tags.map(tag => `<span class="spot-tag">${tag}</span>`).join(' ');

    card.innerHTML = `
        <div class="spot-image">
            <img src="https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Shanghai%20tourism%20itinerary%20${itinerary.title}&image_size=landscape_4_3" alt="${itinerary.title}">
        </div>
        <div class="spot-info">
            <div class="spot-header">
                <h3 class="spot-name">${itinerary.title}</h3>
                <span class="spot-city">${itinerary.days}天</span>
            </div>
            <div class="spot-tags">
                ${tagsHtml}
            </div>
            <div class="spot-details">
                <div class="spot-rating-info">
                    <span class="spot-rating-label">强度：</span>
                    <span class="spot-rating">${itinerary.intensity}/5</span>
                </div>
                <div class="spot-price-info">
                    <span class="spot-price-label">价格：</span>
                    <span class="price">¥${itinerary.total_cost}</span>
                </div>
            </div>
            <div class="spot-open-time">
                <span class="spot-open-time-label">分类：</span>
...
function renderNews() {
    ...
    allNews.forEach(news => {
        ...
        const imageUrl = news.image || 'https://via.placeholder.com/300x150?text=News';
...
function renderNewsList() {
    ...
    allNews.forEach(news => {
        ...
        // 使用 news.json 中的 image 字段
        const imageUrl = news.image || 'https://via.placeholder.com/400x250?text=News';
...
function openEventDrawer(news) {
    ...
    if (posterEl) {
        const imageUrl = news.image || 'https://images.unsplash.com/photo-1496449903678-68ddcb189a24?w=800&h=600&fit=crop';
